...
function updateExpTotal() {
    let t = 0; document.querySelectorAll('.exp-row').forEach(row => { const q = parseFloat(row.querySelector('.exp-item-qty').value) || 0; const p = parseFloat(row.querySelector('.exp-item-price').value) || 0; t += (q * p); });
...
document.getElementById('expense-form').addEventListener('submit', async (e) => {
    ...
    submitBtn.disabled = true; submitBtn.innerHTML = '<span class="material-symbols-outlined animate-spin" style="font-size:1.1rem;vertical-align:middle;">sync</span> GUARDANDO...';
    const items = [];
    document.querySelectorAll('.exp-row').forEach(row => { const desc = row.querySelector('.exp-item-desc').value.trim(); const qty = parseFloat(row.querySelector('.exp-item-qty').value) || 0; const price = parseFloat(row.querySelector('.exp-item-price').value) || 0; if (desc && qty && price) items.push({ description: desc, quantity: qty, unit_price: price }); });
...
async function deleteProdRun(id) { if (confirm('¿ELIMINAR ESTE INGRESO?')) { await apiFetch(`/production/${id}`, { method: 'DELETE' }); loadProductionHistory(); } }
...
async function deleteClient(id) { if (confirm('¿ELIMINAR ESTE CLIENTE?')) { await apiFetch(`/clients/${id}`, { method: 'DELETE' }); loadClients(); } }
